...
if u_tested >= 500:
	...
	for i in do_eval_for:
		...
		temp_ind = np.logical_and(np.logical_and(np.less_equal(accs[i],mean_accs*1.3), np.greater_equal(accs[i],mean_accs*0.7)), np.logical_and(np.less_equal(divs[i],mean_divs*1.3), np.greater_equal(divs[i],mean_divs*0.7)))
...
wr = csv.writer(open("../results/eval_collection_u"+str(u_tested)+"_cv"+str(n_rep)+"_test"+str(n_test)+"_rec"+str(n_rec)+"_"+t_cluster+".csv", 'w'))
wr.writerows(out)
