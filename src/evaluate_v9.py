...
def demo(uid, type = 'w2v', n_train=10, n_test=10):
...
def evaluate(uid, type = 'w2v', n_train = 10, n_test = 10):
	...
	if type=='w2v':
		...
		temp = copy.copy(rating_mat[u_ind])
		rating_mat[u_ind] = np.zeros(rating_mat[u_ind].shape)
		knn_train_ind = np.where(np.isin(item_ids.astype(str), train_seq)==True)[0]
		rating_mat[u_ind][knn_train_ind] = train_rat[np.argsort(train_seq.astype(int))]
#		rating_mat[u_ind][np.where(np.isin(item_ids.astype(str), train_seq)==True)[0]] = train_rat
		user_sim = 1 - pairdis(rating_mat, metric='cosine')
		knn_ind = np.argsort(user_sim[u_ind])[::-1][1:k_+1]
		...
		rating_mat[u_ind][i_to_pred] = i_predicted
		rec = item_ids[np.argsort(rating_mat[u_ind][i_to_pred])[::-1]][:n_rec].astype(str)
		res_acc = accuracy_eval(rec, test_seq)
		res_div = diversity_eval(rec, test_seq)
		rating_mat[u_ind] = temp
		...
		temp = copy.copy(rating_mat[u_ind])
		rating_mat[u_ind] = np.zeros(rating_mat[u_ind].shape)
		cf_train_ind = np.where(np.isin(item_ids.astype(str), train_seq)==True)[0] 
		rating_mat[u_ind][cf_train_ind] = train_rat[np.argsort(train_seq.astype(int))]
		user_sim = 1 - pairdis(rating_mat, metric='cosine')
		mean_user_rating = rating_mat.mean(axis=1)
		ratings_diff = rating_mat - mean_user_rating[:, np.newaxis]
		cf_pred = mean_user_rating[u_ind] + (user_sim[u_ind].dot(ratings_diff) / np.sum(user_sim[u_ind]))
#		cf_pred = predict_cf(rating_mat, user_sim)
		i_to_pred = np.setdiff1d(np.array(range(n_items_total)), cf_train_ind)
		i_to_pred = np.where(rating_mat[u_ind]==0)[0]
		rec = item_ids[np.argsort(cf_pred[i_to_pred])[::-1]][:n_rec].astype(str)
		res_acc = accuracy_eval(rec, test_seq)
		res_div = diversity_eval(rec, test_seq)
		rating_mat[u_ind] = temp
...
train_demo, test_demo, rec_demo = [], [], []
for u in user_sample:
	temp = demo(u)
	train_demo.append(temp[0])
	test_demo.append(temp[1])
	rec_demo.append(temp[2])

wr = csv.writer(open("../results/[demo]_u300_tr_10.csv", 'w'))
wr.writerows(train_demo)
wr = csv.writer(open("../results/[demo]_u300_te_10.csv", 'w'))
wr.writerows(test_demo)
wr = csv.writer(open("../results/[demo]_u300_rec_10.csv", 'w'))
wr.writerows(rec_demo)
print "demo done"
